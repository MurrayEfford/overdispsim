...
# Simulation results have been archived as 

# Efford MG (2025) Simulations of overdispersed activity centres in 
# spatially explicit capture-recapture [Data set]. Zenodo. 
# <https://doi.org/10.5281/zenodo.14873670>

# We retrieve them with the R package zen4R
# Blondel E (2024) _zen4R: Interface to 'Zenodo' REST API_. R package version
#   0.10, <https://CRAN.R-project.org/package=zen4R>
  
require(zen4R)
tmpfolder <- tempdir()
on.exit(unlink(tmpfolder, recursive = TRUE))
download_zenodo(path = tmpfolder, "10.5281/zenodo.14873670")

tmpfolder <- paste0(tmpfolder, '/')
load(file = paste0(tmpfolder, 'sims1.RData'))
load(file = paste0(tmpfolder, 'sims2.RData'))
load(file = paste0(tmpfolder, 'sims3.RData'))

load(file = paste0(tmpfolder, 'sims1f.RData'))
load(file = paste0(tmpfolder, 'sims2f.RData'))
load(file = paste0(tmpfolder, 'sims3f.RData'))

load(file = paste0(tmpfolder, 'sims1M.RData'))
load(file = paste0(tmpfolder, 'sims2M.RData'))
load(file = paste0(tmpfolder, 'sims3M.RData'))
load(file = paste0(tmpfolder, 'sims1fM.RData'))
load(file = paste0(tmpfolder, 'sims2fM.RData'))
load(file = paste0(tmpfolder, 'sims3fM.RData'))

load(file = paste0(tmpfolder, 'sims1MCL.RData'))
load(file = paste0(tmpfolder, 'sims2MCL.RData'))
load(file = paste0(tmpfolder, 'sims3MCL.RData'))

load(file = paste0(tmpfolder, 'sims2CCL.RData'))   # cohesion
load(file = paste0(tmpfolder, 'sims2CfCL.RData'))  # cohesion, fixed N(A)
```
